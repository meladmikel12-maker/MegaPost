...
BEGIN
            SELECT content::jsonb INTO sign_result FROM http_post(
                ...
                new_price := (sign_result->'itemResults'->'items'->0->'offersV2'->'listings'->0->'price'->'money'->>'amount')::float8;
            
new_image := COALESCE(
    sign_result->'itemResults'->'items'->0->'images'->'primary'->'large'->>'url',
    sign_result->'itemResults'->'items'->0->'images'->'primary'->'variants'->0->'large'->>'url'
);
